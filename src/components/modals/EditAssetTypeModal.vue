    ...
    <form @submit.prevent>
      ...
      <div class="flexrow task-types mb1">
        ...
        <combobox
          ...
          :with-margin="false"
          @update:model-value="addImportTaskType"
...
const availableImportTaskTypes = computed(() => {
  const filtered = sortByName(
    taskTypes.value.filter(
      taskType =>
        !form.value.import_task_types.includes(taskType.id) &&
        taskType.for_entity === 'Asset'
    )
  )
  return [
...
const addImportTaskType = id => {
  if (taskTypeMap.value.get(id)) {
    form.value.import_task_types.push(id)
  }
}

const removeImportTaskType = idToRemove => {
  const index = form.value.import_task_types.indexOf(idToRemove)
  if (index >= 0) {
    form.value.import_task_types.splice(index, 1)
  }
}
