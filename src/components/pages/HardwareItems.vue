...
const equipmentToAssignPersons = ref({})
...
const modals = reactive({ del: false, edit: false, users: false })
...
const usedAmounts = computed(() => {
  ...
  activePeople.value.forEach(person => {
    const personItems = linkedHardwareItems.value[person.id] || []
...
const onUsersClicked = hardwareItem => {
  equipmentToAssignPersons.value = hardwareItem
  modals.users = true
}
...
onMounted(async () => {
  ...
  linkedHardwareItems.value = await store.dispatch('loadPersonHardwareItems')
})
